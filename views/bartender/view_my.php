        ...
        <div class="my-block__content">
            ...
            <select id="select_month" onChange="new_date()">
                <?php for($i=0; $i<count($list_month); $i++): ?>
                    ...
                    <option value = "<?php echo $list_month[$i]; ?>" <?php if($month_number == $month) echo 'selected'; ?> > <?php echo $this->array_month[(int)$month_number]; ?></option>
                    <?php endfor ?>
            </select> 
            <?php endif; ?>
            <div class="my-table">
    ...
    <table class="table">
        ...
        <?php for($i=0; $i < count($result); $i++): ?>
            <tr>
                ...
                <?php endif; ?>
                <td> <?php $res = $result[$i]['information_percent'] + $result[$i]['information_tips'] - $result[$i]['information_taxi'] - $result[$i]['information_other']; echo $res; ?> </td>
            </tr> 
        <?php endfor; ?>   
        </tbody>
        <tfoot>
        <tr>
            ...
            <?php endif; ?>
            <td><?php $res = $AVG_day[0]['percent'] + $AVG_day[0]['tips'] - $AVG_day[0]['taxi'] - $AVG_day[0]['other']; echo $res; ?></td>
        </tr>
        <tr>
            <th>Итог</th>
            ...
            <?php endif; ?>
            <td><?php $res = $result_outcome[0]['percent'] + $result_outcome[0]['tips'] - $result_outcome[0]['taxi'] - $result_outcome[0]['other']; echo $res; ?></td>
        </tr>
        </tfoot>
    </table>
    <?php else:?>
