<?php
	...
	class my extends CI_Head_controller{
		...
		public function index(){
			...
			$this->db->query("SELECT SUM(`information_percent`) AS `percent`, SUM(`information_kassa`) AS `kassa`, SUM(`information_tips`) AS `tips`, SUM(`information_taxi`) AS `taxi`, SUM(`information_rub`) AS `rub`, SUM(`information_other`) AS `other` FROM `information` WHERE MONTH(`information_date`) = '$month' AND `users_id` = '$user_id'");

			$result_outcome = $this->db->result();

			$this->db->query("SELECT ROUND(AVG(`information_percent`), 0) AS 'percent', ROUND(AVG(`information_kassa`), 0) AS 'kassa', ROUND(AVG(`information_tips`), 0) AS 'tips', ROUND(AVG(`information_taxi`), 2) AS 'taxi', ROUND(AVG(`information_rub`), 0) AS 'rub', ROUND(AVG(`information_other`), 0) AS 'other' FROM `information` WHERE `users_id` = '{$this->get_id()}' AND MONTH(`information_date`) = '{$month}';");
